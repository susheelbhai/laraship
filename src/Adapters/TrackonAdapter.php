...
class TrackonAdapter implements ShippingProviderInterface
{
    private string $baseUrl = 'https://api.trackon.in';
    ...
    public function __construct(
        ...
        if (empty($this->apiKey) || empty($this->customerId)) {
            throw new ProviderAuthenticationFailedException('Trackon API key and customer ID are required');
        }
    }
    ...
    public function calculateRates(ShippingRateRequest $request): array
    {
        try {
            ...
            if ($response->failed()) {
                throw new ShippingException('Failed to fetch rates from Trackon');
            }
            ...
            throw new ShippingException('Trackon rate calculation failed: '.$e->getMessage());
        }
    }
    ...
    public function bookCourier(CourierBookingRequest $request): CourierBookingResponse
    {
        try {
            ...
            if ($response->failed()) {
                throw new ShippingException('Trackon booking failed: '.$response->body());
            }

            $data = $response->json();

            if (! isset($data['cn_number'])) {
                throw new ShippingException('No CN number received from Trackon');
            }
            ...
            throw new ShippingException('Trackon booking failed: '.$e->getMessage());
        }
    }
    ...
    public function getTrackingInfo(string $trackingNumber): array
    {
        try {
            ...
            if ($response->failed()) {
                throw new ShippingException('Failed to fetch tracking info from Trackon');
            }

            return $response->json();

        } catch (\Exception $e) {
            throw new ShippingException('Trackon tracking failed: '.$e->getMessage());
        }
    }
    ...
    public function schedulePickup(array $shipmentIds, \DateTime $pickupDate): array
    {
        return [
            'success' => true,
            'message' => 'Trackon requires manual pickup scheduling',
        ];
    }
    ...
    public function cancelShipment(string $trackingNumber): bool
    {
        try {
            ...
            throw new ShippingException('Trackon cancellation failed: '.$e->getMessage());
        }
    }
